...
import { Bars3Icon, MoonIcon, ShoppingBagIcon, UserCircleIcon, XMarkIcon } from '@heroicons/react/24/solid';
...
const Header = () => {
    ...
    const { countProducts, search, setSearch, setUrlProduct } = useUserContext();
    const [isOpenMenu, setIsOpenMenu] = useState(false);
    const [openToogle, setOpenToogle] = useState(false);

    const activesStyle = 'bg-background-blue text-text-ligth rounded p-1 text-sm cursor-pointer';
         ...
         <header className='shadow-md w-full fixed top-0 left-0'>
            <div className='md:flex  items-center justify-between bg-white py-2 md:px-10 px-4 '>
                <div className='flex md:w-11/12  w-full justify-between items-center'>
                    <img src={logo} alt="Logo Radcars" className='md:w-24 md:h-20 w-20 h-18 pt-2 pl-2'/>
                    <input 
                        type='search'
                        placeholder='Search'
                 ...
                 <div className="flex gap-4 items-center justify-end pb-2 sm:pb-0 md:w-2/12 w-full">
                     <Link to={'shopping'}>
                         <p className="flex justify-center">
                            ...
                            {
                                countProducts !== 0 &&
                                <span className="absolute text-text-ligth bg-background-blue rounded-3xl h-4 w-4 text-md p-1 items-center flex justify-center mt-3 font-light text-sm">{countProducts}</span>
                            }
                     ...
                     <MoonIcon className="h-6 w-6 hover:text-text-blue"/>
                    {
                        ...
                        :
                        <XMarkIcon onClick={openOrCloseMenu}
                        className="h-6 w-6 hover:text-text-blue md:hidden duration-500 transition-all "/>
                    }
                 </div> 
            </div>
            <nav>
                <ul className={`md:flex md:items-center pb-6 absolute md:static bg-white md:z-auto z-[-1] left-0 w-full md:w-full md:justify-center md:pb-4 md:pl-0 pl-9 transition-all duration-500 ease-in ${isOpenMenu ? 'top-20 shadow-md':'top-[-490px]'}`}>
                    {
                    Links.map((link)=>(
                ...
                <div 
                    onClick={openOrCloseMenu}
                    className={`${isOpenMenu && 'bg-slate-300 relative min-h-screen sm:top-68 top-64 md:top-0 opacity-0'}`}></div>
            </nav>         
        </header>
    )
};
