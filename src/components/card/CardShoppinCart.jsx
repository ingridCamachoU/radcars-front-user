...
const CardShoppinCart = ({name, images, net_price, quantity, onAddProduct, decrase, deleteProduct}) => {
        ...
        <div className='items-center flex justify-between mt-8 bg-white p-2 flex-wrap sm:flex-row flex-col w-full sm:gap-0 gap-4'>
            <div className='flex sm:w-8/12 w-full items-center'>
                <picture className='sm:w-1/5 w-1/3'>
                    <img className='w-16 h-16' src={images.length > 0 ? images : sinImagen}  alt="sin imagen"/>
                </picture>
            ...
            <div className='flex gap-2 sm:w-4/12 w-full items-center justify-between'>
                <div className='flex gap-1 bg-slate-100 border shadow-sm p-1 rounded'>
                    <button className='border-r  hover:bg-slate-200 cursor-pointer disabled:text-gray-500 disabled:pointer-events-none active:scale-95 px-2' disabled={quantity===1} onClick={decrase}>-</button>
                    <p>{quantity}</p>
                    <button className='border-l px-2  hover:bg-slate-200 cursor-pointer active:scale-95 disabled:text-gray-500 disabled:pointer-events-none' disabled={quantity>=10} onClick={onAddProduct}>+</button>
                </div>  
