...
const ShoppingCart = () => {
    ...
    const {cart, onAddProduct, setCart, decrase, deleteProduct, total, countProducts} = useUserContext();
    ...
    return (
        <LayoutBase>
            <div className='w-full max-w-screen-xl'>
                            ...
                            ?   
                                <div className='bg-white text-text-gray p-1     items-center flex flex-col gap-2'>
                                    <img  
                                        ...
                                        alt='shopping'
                                        className='w-40 h-40' />
                                    <h3 className='font-bold'>¡Empieza un carrito de compras! </h3>
                                    <p className='m-2'>Una vez que añadas algo a tu carrito, aparecerá aquí. ¿Listo para empezar?</p> 
                                </div> 
                            : (cart.map)( product =>
                                (
                                    <CardShoppinCart key={product.id} cart={cart} setCart={setCart}
                    ...
                    <div className='text-text-gray bg-white lg:w-3/12 w-11/12 px-6 py-4 shadow-md m-4 h-72 items-center flex justify-center'>
                        {
                            ...
                            ? 
                                <div className='mt-2 text-text-gray'> 
                                    <h5 className='mb-4 font-bold'>Resumen de Compra</h5> 
                                    <p>Aquí verás los importes de tu compra una vez que agregues productos.</p>
                                </div> 
                            : 
                                <div className='gap-4 flex flex-col px-4'>
                                    <h5 className='font-bold text-lg'>Resumen del Predido</h5>
                                    <p className='font-light'>
                                        <span>{countProducts} productos</span>
                                    </p>
                                    <p className='flex gap-8 mt-2'>
                                        <span >Total:</span> <span className='font-semibold'>$ {converterPrice(price)}</span>
                                    </p>
                                    <button className='bg-green-500 text-text-ligth items-center flex justify-center gap-2 rounded-lg w-50 mt-4 hover:bg-green-600 px-1 '>Continuar compra 
                                    <span><FontAwesomeIcon icon={faWhatsapp} className='h-8 w-8 text-text-ligth m-1'/></span></button>
                                </div>
                        }
                    </div>
                </div>
                ...
                <h3 className='text-lg text-text-gray font-bold my-8 flex items-center justify-center'>También te puede interesar</h3>
                <section className='w-full bg-background-blue py-10 flex px-6 my-6'>
                    <Sliders url={urlProduct} data={data} loadingData={loadingData} loading={loading}/>
                </section>
